...
I provide replicable code [in this repository](https:/github.com/xmarquez/GPTDemocracyIndex) using the \`[targets](https://books.ropensci.org/targets/)\` package, but follow along for an explanation and an evaluation of the results. If you want to skip the technical details of R code, just scroll down to the section on evaluation.
...
Note that since ChatGPT has a knowledge cutoff of 2021, it does not make sense to ask it to generate democracy scores for after that date. To keep costs low and be able to do some experimentation, I'm also only calculating the scores for 20% of all country years - here, simply the years that end in 0 and 5. (A full run of all `r nrow(fh_data)` country-years only costs about \$8, but I did some trial-and error testing and some experimentation, and with a single API key coding `r nrow(fh_panel)` country-years will in any case take about 6 hours to complete for the GPT3.5 API).

We also need to craft a good a prompt. The [OpenAI API documentation for the Chat GPT models](https://platform.openai.com/docs/guides/chat/introduction) indicates that a prompt should have at leas one "system" message and one "user" message. The system message gives the system an identity to emulate (though OpenAI also notes that GPT3.5 does not always pay particular attention to the system message); the user message contains your instructions.
...
Finally, I also experimented with a different prompt based on the [Democracy/Dictatorship dataset](https://xmarquez.github.io/democracyData/reference/pacl.html), running `r nrow(fh_panel)` country-years through OpenAI:

> A democracy is a system where 1) the top executive is elected (whether directly by popular vote or indirectly by popularly elected legislators); 2) there is a legislature with members directly or indirectly elected; 3) There are no major restrictions on popular suffrage; 4) Multiple parties are legally allowed and actually compete as independent entities (not just as members of a Popular Front controlled by a single party); 5) Multiple parties are actually represented in the legislature; 6) the current incumbent has not unconstitutionally closed the legislature and rewritten the rules in their favor; and 7) there has been or it is obvious that there could be an alternation of power through elections, where the current executive would lose power and an opposition party would take over.
>
> Based on this definition and your knowlege of the politics of \[country X\] in \[year Y\], was \[country X\] a democracy in \[year Y\]?
>
> Use only knowledge relevant to \[country X\] in \[year Y\] and answer simply yes or no. Add a measure of your confidence from 0 to 1, with 0 representing "not confident at all", and 1 representing "very confident". Use this template to answer: \[country X\] in \[year Y\]: yes or no, confidence: number from 0-1, justification: brief justification referring back to the 7 conditions above.
...
A simple hierarchical cluster analysis (@fig-cluster) confirms that the AI-generated scores (especially GPT3.5's scores) are closest in their judgments to the Varieties of Democracy "additive polyarchy index" (a variant of the Polyarchy index).
...
These correlations vary across time and space, however. In @fig-correlations-year-openai and @fig-correlations-year-claude, we see that these tend to be higher in more recent years and lower in the 1970s (though for Claude there's a bit of a U shape).
...
We can also see where the AI-generated scores produce idiosyncratic judgments. Below, I calculate which countries have the highest and the lowest Root Mean Squared Errors (RMSE) between the scaled (0-1) AI score and the V-Dem Additive Polyarchy Index (@tbl-worst-cases-for-openai).
...
By contrast, GPT3.5 does especially well with established democracies (@tbl-best-cases-for-openai):
...
Indeed, it's clear that both models tend to struggle with countries with "hybrid" institutions (apparently democratic but with significant problems), which tend to be hard for human coders too, as is evident from @fig-vdem-rmse. But GPT3.5 struggles less!
...
In any case, Claude struggles with an entirely different set of countries than GPT3.5 ( @tbl-worst-cases-for-claude).
...
Overall, GPT3.5 has the edge in accuracy over Claude, with an overall RMSE relative to the V-Dem Additive Polity Index of `r rmse_summary$rmse[rmse_summary$measure == "openai_score"]` vs. `r rmse_summary$rmse[rmse_summary$measure == "claude_score_2"]` .

## How good are their justifications?
...
```{r}
#| echo: false
#| tbl-label: venezuela-justifications-openai
...
```{r}
#| echo: false
#| tbl-label: venezuela-justifications-claude
...
The V-Dem project already exists and publishes annually updated estimates of democracy scores, complete with component indicators and all index subcomponents. While using ChatGPT to replicate these does not add much value right now, it seems clear to me that over time it will become more and more feasible to use these models to generate political science data. Given the pace of change in this field, I wouldn't bet against these models become more and more important for the data-generating process, especially in a hybrid model where experts review their judgments. For example, I am currently working on generating data that extends the [Geddes, Wright, and Frantz](https://sites.psu.edu/dictators/ ) classification of non-democratic regimes; it would be cool if this worked well.

## Final Data

I've made the final scores generated by both models (GPT3.5 and Claude) available here for people interested in analyzing it further.
